...
var bs = {
    ...
    game: false,
    stop: false,
    level: true, //Уровень сложности бота.
        ...
        run: function(){
            if(!bs.stop) {
                ...
                if (itm.id == 1){
                    ...
                    }else bs.status("Вы проиграли, хотите отыграться ?");
                } else {
                    bs.status("Ваш ход !!!");
    ...
    getPositionClone: function(){
        ...
        for(var i=0; i<itms.length; i++){
            var left = itms[i].offsetLeft;
            var top = itms[i].offsetTop;
            $(".clone:eq("+i+")").attr("data-left",left).attr("data-top",(top+plus));
            $(".clone:eq("+i+") div[ship][pos]").css({
                top: top+plus,
                left: left
            });
        }
    ...
    ready: function(bot,callback){
        ...
        if(bool){
            html.addClass("start");
            bs.status("Нажмите Play для начала игры..");
        }else{
    ...
    repeat:function(){
        ...
        bs.get.cleanItm(".itm");//Очищаем HTML

        //Удаляем якори..
        $(".body.field")
            .removeClass("game")
            .removeClass("stop");
        $(".ship-list").removeClass("start");
        $(".my-field .canvas").addClass("off");

        bs.allGoBack();//Возвращаем корабли на место..

        //Загружаем игру по новой..
        i.stop = false;
        i.player = true;
        i.check_res = [];
        i.field = [];
